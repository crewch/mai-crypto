...
/*The API functions*/
HashReturn Init(hashState *state, int hashbitlen, int roundCount);
...
void F8(hashState *state)
{
      ...
      int roundCount = state->roundCount;
      /*perform 42 rounds*/
      for (i = 0; i < roundCount; i = i + 7)
      {
            round_function(00, i);
...
int calculateHash(char *text, int roundCount, unsigned char *hashResult)
{
      hashState state;

      if (Init(&state, 256, roundCount) != SUCCESS)
      {
            printf("Ошибка при инициализации состояния хэша.\n");
            return 1;
      }

      if (Update(&state, (unsigned char *)text, strlen(text)) != SUCCESS)
      {
            printf("Ошибка при обновлении хэша.\n");
            return 1;
      }

      if (Final(&state, hashResult) != SUCCESS)
      {
            printf("Ошибка при завершении вычисления хэша.\n");
            return 1;
      }
}

void flipBitsInString(char *text, int size, int bits, char *result)
{
      int bitIndex = 0;
      ...
      for (int i = 0; i < size && bitIndex < bits; i++)
      {
            for (int j = 0; j < 8 && bitIndex < bits; j++)
            {
                  if ((result[i] & (1 << j)) == 0)
                  {
                        result[i] |= (1 << j);
...
int countDifferentBits(const char *str1, const char *str2, int size)
{
      int count = 0;
...
void compareHashes(char *text, int bitsToChange, int roundCount) {
    ...
    for (int i = 1; i <= bitsToChange; i++) {
        for (int j = 1; j <= roundCount; j++) {
            if (calculateHash(text, j, originalHash) != 0) {
                return;
            }

            flipBitsInString(text, strlen(text), i, modifiedText);

            if (calculateHash(modifiedText, j, modifiedHash) != 0) {
                return;
            }

            int diffBits = countDifferentBits((const char *)originalHash, (const char *)modifiedHash, 32);
...
int main()
{
      int roundCount, bitsToChange;
      char text[1000];

      printf("Введите количество раундов: ");
      scanf("%d", &roundCount);

      printf("Введите количество битов которые надо менять: ");
      scanf("%d", &bitsToChange);

      printf("Введите текст для хэширования: ");
      scanf(" %[^\n]s", text);

      compareHashes(text, bitsToChange, roundCount);
